...
class CreateSizeTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->attributes = make(Size::class)->toArray();
    }
    ...
    public function test_authenticated_users_with_the_admin_role_can_create_sizes()
    {
        ...
        $size = make(Size::class);
        $this->postJsonEndpoint($size->toArray(), $this->generateAuthHeaders())
            ->assertStatus(200)
            ->assertSee("$size->price");

        $this->assertDatabaseHas('sizes', $size->toArray());
    }
    ...
    public function test_all_fields_required_are_passed_on_request()
    {
        ...
        $test = function ($attributes) {
            $this->postJsonEndpoint($attributes, $this->generateAuthHeaders())
                ->assertStatus(422)
                ->assertJsonStructure([
                    "message",
